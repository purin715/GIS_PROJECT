...
st.title("Haagen-Dazs X 팔칠구삼")
st.header("1. 이미지를 업로드하세요.")
...
image_file = st.file_uploader(label='', type=['png', 'jpg'])
...
if image_file: 
    st.header("")
    st.header("2. 어떤 단어의 폰트를 확인하고 싶으신가요?")
    ...
    # 원하는 이미지 선택
    option = st.radio('', range(1, number+1))
    if option:
...
####################
    ...
    st.header("")
    st.header("3. 텍스트가 잘 안잘렸다면 마우스로 직접 지정해보세요.")
...
#############
    ...
    if image_file is not None:
        ...
        st.subheader('▶ 위 단어와 가장 유사한 폰트는 {} 입니다.'.format(counts.most_common(1)[0][0]))
